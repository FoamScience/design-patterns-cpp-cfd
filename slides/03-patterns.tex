...
\node<2> (factory-pic) at (factory.center -| creational.east) [picst] {Create concreate objects through a base class};
...
\node<6> (proxy-pic2) at (proxy.center -| structural.west) [picst3] {
    ...
    \end{class}
    \draw[dotted] (2.5, -4) -- (fvMeshSubsetProxy.north);
...
\node<7> (adapter-pic2) at (adapter.center -| structural.west) [picst3]
{
    ...
    \begin{class}[text width=4.5cm]{CPLSocketLAMMPS}{2.5,-2}
        ...
        \operation{+ unpack() : void}
    \end{class}
    \draw[dotted] (2.5, 0.5) -- (CPLSocketFOAM.south);
    \draw[dotted] (2.5, 0.5) -- (CPLSocketLAMMPS.north);
...
\node<11> (iterator-pic2) at (iterator-pic.west) [picst5]
{
    ...
    \begin{class}[text width=2.5cm]{iterator}{4,-3}
        ...
        \operation{+ good() : bool}
    \end{class}
    \draw[dotted] (-7,-2.5) -- (LinkedList<T>.north);
    \draw[dotted] (-7,-2.5) -- (UList<T>.north);
...
\node<12> (strategy-pic2) at (strategy-pic.west) [picst5]
{
    ...
    \begin{class}[text width=4cm]{gaussDivScheme<T>}{2,-1}
        \operation{+ fvcDiv(Field\&) : tmp<Field<T>>}
    \end{class}
    \draw[dotted] (-7,-2) -- (gaussDivScheme<T>.north);
    \umlnote ( note ) at (-7, -2)
...
\node<13> (observer-pic2) at (observer-pic.west) [picst5]
{
    ...
    \begin{class}[text width=3cm]{dynamicFvMesh}{-8,7.5}
        \operation[0]{+ update() : void}
    \end{class}
    \draw[dotted] (-7,-1) -- (MeshObject.north);
    \umlnote ( note ) at (-7, -1)
        {in \mintinline{cpp}{dynamicFvMesh::update()} \\ \mintinline{cpp}{for(auto& obj : objs)}\\ \mintinline{cpp}{{obj.updateMesh(mesh, map)}}};
    \renewcommand{\umltextcolor}{mDarkTeal}
    \renewcommand{\umldrawcolor}{mDarkTeal}
...
\node<14> (visitor-pic2) at (visitor-pic.west) [picst5]
{
    ...
    \end{class}
    \begin{class}[text width=4cm]{lduMatrix}{-5,4}
...
\end{tikzpicture}
\end{center}
\caption{Design pattern examples from OpenFOAM}\label{fig:01}
\end{figure}
